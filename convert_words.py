...
class SktNumToWord:
    def __init__(self, **kwargs):
        ...
        skt_num = kwargs.get("skt_num")
        if not skt_num and not number:
            raise InvalidInputException(
                "Either integer should be given \
                    as an input or Sanskrit numerals\
                         should be provided."
            )
        if skt_num and type(skt_num) != str:
            raise InvalidInputException(
                "Sanskrit numeral input\
                should be a string."
            )
        ...
        if not number:
            number = int("".join([HINDU_NUMS.get(dig) for dig in skt_num]))
        self.number = number
        self.num_dict = {
            0: {
                0: "",
